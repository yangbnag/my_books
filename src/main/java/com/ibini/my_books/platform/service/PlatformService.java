...
public class PlatformService {
    ...
    public boolean setPlatformForNewMember(String account) {
        ...
        log.info("회원가입시 플랫폼 에러 확인 bgMap : {}",bgMap);

        for (String s : bgMap.keySet()) {
            log.info("회원가입시 플랫폼 에러 확인 s : {}",s);
            PlatformDomain p = new PlatformDomain();
            log.info("회원가입시 플랫폼 에러 확인 p : {}",p);
            ...
            log.info("bgMap.get(s)[0] : {}",bgMap.get(s)[0]);
            log.info("회원가입시 플랫폼 에러 확인 p : {}",p);
            p.setPlatformFontColor(bgMap.get(s)[1]);
            log.info("bgMap.get(s)[1] : {}",bgMap.get(s)[1]);
            log.info("회원가입시 플랫폼 에러 확인 p : {}",p);


            platformMapper.savePlatform(p);
            log.info("회원가입시 플랫폼 에러 확인 platformDomain : {}",p);
        }
